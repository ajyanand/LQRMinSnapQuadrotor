...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        if self.path is None:
            ...
            # STUDENT CODE HERE
            self.travelvel = 2.65 # approximate, just to get times for each segment 2.75
            ...
            for i in range(0, n):
                ...
                Gi, hi = addCorridorConstraints(dt[i], self.points[i], self.points[i+1], 1, np.sqrt(self.eps**2 / 3), d, n, i)
            ...
            QPResults = cvxopt.solvers.qp(cvxopt.matrix(H), cvxopt.matrix(np.zeros((4*d*n, 1))), cvxopt.matrix(G), cvxopt.matrix(h),
