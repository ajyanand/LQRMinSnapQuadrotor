...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        if self.path is None:
            ...
            # STUDENT CODE HERE
            self.travelvel = 2.75  # approximate, just to get times for each segment
            ...
            QPResults = cvxopt.solvers.qp(cvxopt.matrix(H), cvxopt.matrix(np.zeros((4*d*n, 1))), None, None,
                                               cvxopt.matrix(Aeq), cvxopt.matrix(beq))
            ...
            for i in range(1, n+1):
                tmat[i] = tmat[i-1] + dt[i-1]

            self.tlist = tmat[1:]

            print("BOOM")
            '''
            #temporary linear placeholder
            self.travelvel = 2.75  # approximate, just to get times for each segment
            self.numWaypoints = self.points.shape[0]
            self.currWaypointTimeOffset = 0
            self.coeffList =[]
            t0 = 0
            v0 = np.zeros((3, 1))
            vf = np.zeros((3, 1))
            ...
            if self.numWaypoints == 1:
                q0 = np.array([[self.points[0, 0]], [self.points[0, 1]], [self.points[0, 2]]])
                qf = q0
                tf = 0.01
                polycoeff = (qf - q0) / (tf - t0)
                self.coeffList.append([tf, polycoeff, q0])

            else:
                for i in range (0, self.numWaypoints-1):
                    q0 = np.array([[self.points[i, 0]], [self.points[i, 1]], [self.points[i, 2]]])
                    qf = np.array([[self.points[i+1, 0]], [self.points[i+1, 1]], [self.points[i+1, 2]]])
                    #Got a mystery duplicate point, so I'm trying this to get around it for now
                    if np.all(qf==q0):
                        qf = np.array([[self.points[i+2, 0]], [self.points[i+2, 1]], [self.points[i+2, 2]]])

                    tf = t0 + (np.absolute(np.linalg.norm(qf-q0))/self.travelvel)
                    polycoeff=(qf-q0)/(tf-t0)
                    self.coeffList.append([tf, polycoeff, q0])
                    t0 = tf
                self.coeffList.append([tf, qf*0, qf])
            '''
    ...
    def update(self, t):
        ...
        # if the current time is less than the tf of the first segment
...
def dougPeuck(points, margin):
    ...
    # if all the points are within epsilon of the line between the start and end
...
def calcAb_i1(i, n, d, dt_i, w_i, w_ip1):
    ...
    startind = i*4*d
    # little unsure of this indexing
    # wi=sig_i,0
...
def calcAb_i2k(i, k, n, d, dt_i):
    ...
    startind = (4*d*i)+(4*k)
    endind = startind + 4*len(j)#CHECK INDEX
...
def calcSnapPoly(coeff, t):
    #returns x through it's 4th derivative, evaluated at time t with the provided coefficients
    ...
    xddd = 6 * coeff[3] + 24 * coeff[4] * (t) + 60 * coeff[5] * (t ** 2) + 120 * coeff[6] * (t ** 3) + 210 * coeff[7] * (t ** 4)
